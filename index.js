...
function getWinnersByYear(array, getyearscb, getwinnerscb) {
  const yearsString = getyearscb(array);
  const winnersString = getwinnerscb(array);
  const bothStrings = [];
  for (let i = 0; i < yearsString.length; i++) {
    bothStrings.push(
      `In ${yearsString[i]}, ${winnersString[i]} won the world cup!`
    );
  }
  return bothStrings;
}
